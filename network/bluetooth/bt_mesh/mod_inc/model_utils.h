...
static inline void
model_transition_buf_add(struct mesh_net_buf_simple *buf,
			 const struct bt_mesh_model_transition *transition)
{
	mesh_net_buf_simple_add_u8(buf, model_transition_encode(transition->time));
	mesh_net_buf_simple_add_u8(buf, model_delay_encode(transition->delay));
}

static inline void
model_transition_buf_pull(struct mesh_net_buf_simple *buf,
			  struct bt_mesh_model_transition *transition)
{
	transition->time = model_transition_decode(mesh_net_buf_simple_pull_u8(buf));
	transition->delay = model_delay_decode(mesh_net_buf_simple_pull_u8(buf));
}
