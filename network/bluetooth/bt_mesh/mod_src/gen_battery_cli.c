...
int bt_mesh_gen_bat_decode_status(struct mesh_net_buf_simple *buf,
				  struct bt_mesh_battery_status *status)
{
	uint8_t battery_lvl =mesh_net_buf_simple_pull_u8(buf);
	...
	uint8_t *discharge_minutes =mesh_net_buf_simple_pull_mem(buf, 3);

	status->discharge_minutes =
		((discharge_minutes[2] << 16) | (discharge_minutes[1] << 8) |
		 (discharge_minutes[0]));

	uint8_t *charge_minutes =mesh_net_buf_simple_pull_mem(buf, 3);

	status->charge_minutes =
		((charge_minutes[2] << 16) | (charge_minutes[1] << 8) |
		 (charge_minutes[0]));

	uint8_t flags =mesh_net_buf_simple_pull_u8(buf);
...
#endif /* CONFIG_BT_MESH_GEN_BATTERY_CLI */
