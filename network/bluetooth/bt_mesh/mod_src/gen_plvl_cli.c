				...
				struct mesh_net_buf_simple *buf)
{
	...
	status.current =mesh_net_buf_simple_pull_le16(buf);
	if (buf->len == 3) {
		status.target =mesh_net_buf_simple_pull_le16(buf);
			       ...
			       struct mesh_net_buf_simple *buf)
{
	...
	struct bt_mesh_plvl_cli *cli = model->user_data;
	uint16_t last =mesh_net_buf_simple_pull_le16(buf);
				  ...
				  struct mesh_net_buf_simple *buf)
{
	...
	struct bt_mesh_plvl_cli *cli = model->user_data;
	uint16_t default_lvl =mesh_net_buf_simple_pull_le16(buf);
			       ...
			       struct bt_mesh_plvl_status *rsp)
{
	...
	mesh_net_buf_simple_add_le16(&buf, set->power_lvl);
			       ...
			       struct bt_mesh_plvl_range_status *rsp)
{
	...
	mesh_net_buf_simple_add_le16(&buf, range->min);
	mesh_net_buf_simple_add_le16(&buf, range->max);
				 ...
				 uint16_t default_power, uint16_t *rsp)
{
	...
	mesh_net_buf_simple_add_le16(&buf, default_power);
...
#endif /* CONFIG_BT_MESH_GEN_POWER_LEVEL_CLI */