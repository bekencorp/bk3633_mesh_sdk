			      ...
			      struct mesh_net_buf_simple *buf)
{
	...
	status.current_light =mesh_net_buf_simple_pull_le16(buf);
	status.current_temp =mesh_net_buf_simple_pull_le16(buf);
	...
	if (buf->len == 5) {
		status.target_light =mesh_net_buf_simple_pull_le16(buf);
		status.target_temp =mesh_net_buf_simple_pull_le16(buf);
				     ...
				     struct mesh_net_buf_simple *buf)
{
	...
	status.status =mesh_net_buf_simple_pull_u8(buf);
	status.range.min =mesh_net_buf_simple_pull_le16(buf);
	status.range.max =mesh_net_buf_simple_pull_le16(buf);
			       ...
			       struct mesh_net_buf_simple *buf)
{
	...
	status.current.temp =mesh_net_buf_simple_pull_le16(buf);
	if ((status.current.temp < BT_MESH_LIGHT_TEMP_MIN) ||
	    (status.current.temp > BT_MESH_LIGHT_TEMP_MAX)) {
		return;
	}

	status.current.delta_uv =mesh_net_buf_simple_pull_le16(buf);

	if (buf->len == 5) {
		status.target.temp =mesh_net_buf_simple_pull_le16(buf);
		if ((status.target.temp < BT_MESH_LIGHT_TEMP_MIN) ||
		    (status.target.temp > BT_MESH_LIGHT_TEMP_MAX)) {
			return;
		}

		status.target.delta_uv =mesh_net_buf_simple_pull_le16(buf);
		status.remaining_time =
			model_transition_decode(mesh_net_buf_simple_pull_u8(buf));
	} else {
				  ...
				  struct mesh_net_buf_simple *buf)
{
	...
	status.light =mesh_net_buf_simple_pull_le16(buf);
	status.temp =mesh_net_buf_simple_pull_le16(buf);
	if ((status.temp < BT_MESH_LIGHT_TEMP_MIN) ||
	    (status.temp > BT_MESH_LIGHT_TEMP_MAX)) {
		return;
	}

	status.delta_uv =mesh_net_buf_simple_pull_le16(buf);
...
static int bt_mesh_light_ctl_cli_init(struct bt_mesh_model *model)
{
	...
	mesh_net_buf_simple_init_with_data(&cli->pub_buf, cli->pub_data,
				      sizeof(cli->pub_data));
			  ...
			  struct bt_mesh_light_ctl_status *rsp)
{
	...
	mesh_net_buf_simple_add_le16(&msg, set->params.light);
	mesh_net_buf_simple_add_le16(&msg, set->params.temp);
	mesh_net_buf_simple_add_le16(&msg, set->params.delta_uv);
				  ...
				  struct bt_mesh_light_ctl *rsp)
{
	...
	mesh_net_buf_simple_add_le16(&msg, set->light);
	mesh_net_buf_simple_add_le16(&msg, set->temp);
	mesh_net_buf_simple_add_le16(&msg, set->delta_uv);
	...
	struct bt_mesh_light_temp_range_status *rsp)
{
	...
	mesh_net_buf_simple_add_le16(&msg, set->min);
	mesh_net_buf_simple_add_le16(&msg, set->max);
