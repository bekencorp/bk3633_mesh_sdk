...
static inline void
bt_mesh_loc_global_encode(struct mesh_net_buf_simple *buf,
			  const struct bt_mesh_loc_global *loc)
{
	...
	mesh_net_buf_simple_add_le32(buf, latitude);
	mesh_net_buf_simple_add_le32(buf, longitude);
	mesh_net_buf_simple_add_le16(buf, loc->altitude);
}

static inline void bt_mesh_loc_global_decode(struct mesh_net_buf_simple *buf,
					     struct bt_mesh_loc_global *loc)
{
	loc->latitude =
		(mesh_net_buf_simple_pull_le32(buf) / BT_MESH_LOC_LATITUDE_FACTOR);
	loc->longitude =
		(mesh_net_buf_simple_pull_le32(buf) / BT_MESH_LOC_LONGITUDE_FACTOR);
	loc->altitude =mesh_net_buf_simple_pull_le16(buf);
}
...
static inline void bt_mesh_loc_local_encode(struct mesh_net_buf_simple *buf,
					    const struct bt_mesh_loc_local *loc)
{
	mesh_net_buf_simple_add_le16(buf, loc->north);
	mesh_net_buf_simple_add_le16(buf, loc->east);
	mesh_net_buf_simple_add_le16(buf, loc->altitude);
	...
	mesh_net_buf_simple_add_u8(buf, enc_floor);
	...
	mesh_net_buf_simple_add_le16(buf, uncertainty);
}

static inline void bt_mesh_loc_local_decode(struct mesh_net_buf_simple *buf,
					    struct bt_mesh_loc_local *loc)
{
	loc->north =mesh_net_buf_simple_pull_le16(buf);
	loc->east =mesh_net_buf_simple_pull_le16(buf);
	loc->altitude =mesh_net_buf_simple_pull_le16(buf);

	uint8_t enc_floor =mesh_net_buf_simple_pull_u8(buf);
	...
	uint16_t uncertainty =mesh_net_buf_simple_pull_le16(buf);
