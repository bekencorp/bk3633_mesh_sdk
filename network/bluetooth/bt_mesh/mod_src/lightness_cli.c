...
static void light_status(struct bt_mesh_model *model, struct bt_mesh_msg_ctx *ctx,
			 struct mesh_net_buf_simple *buf, enum light_repr repr)
{
	...
	status.current = repr_to_light(mesh_net_buf_simple_pull_le16(buf), repr);
	if (buf->len == 3) {
		status.target =
			repr_to_light(mesh_net_buf_simple_pull_le16(buf), repr);
			       ...
			       struct mesh_net_buf_simple *buf)
{
	...
	struct bt_mesh_lightness_cli *cli = model->user_data;
	uint16_t last = repr_to_light(mesh_net_buf_simple_pull_le16(buf), ACTUAL);
				  ...
				  struct mesh_net_buf_simple *buf)
{
	...
	uint16_t default_lvl =
		repr_to_light(mesh_net_buf_simple_pull_le16(buf), ACTUAL);
				...
				struct mesh_net_buf_simple *buf)
{
	...
	status.range.min = repr_to_light(mesh_net_buf_simple_pull_le16(buf), ACTUAL);
	status.range.max = repr_to_light(mesh_net_buf_simple_pull_le16(buf), ACTUAL);
...
// 	mesh_net_buf_simple_reset(model->pub->msg);
			    ...
			    struct bt_mesh_lightness_status *rsp)
{
	...
	mesh_net_buf_simple_add_le16(&buf, set->lvl);
	mesh_net_buf_simple_add_u8(&buf, cli->tid++);
	if (set->transition) {
				    ...
				    struct bt_mesh_lightness_range_status *rsp)
{
	...
	mesh_net_buf_simple_add_le16(&buf, light_to_repr(range->min, ACTUAL));
	mesh_net_buf_simple_add_le16(&buf, light_to_repr(range->max, ACTUAL));
				      ...
				      uint16_t default_light, uint16_t *rsp)
{
	...
	mesh_net_buf_simple_add_le16(&buf, light_to_repr(default_light, ACTUAL));
