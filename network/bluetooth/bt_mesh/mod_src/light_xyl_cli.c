...
static int status_decode(struct bt_mesh_light_xyl_cli *cli,
			 ...
			 struct mesh_net_buf_simple *buf, uint32_t opcode,
			 struct bt_mesh_light_xyl_status *status)
{
	...
	status->params.lightness =mesh_net_buf_simple_pull_le16(buf);
	status->params.xy.x =mesh_net_buf_simple_pull_le16(buf);
	status->params.xy.y =mesh_net_buf_simple_pull_le16(buf);
	status->remaining_time =
		(buf->len == 1) ?
			model_transition_decode(mesh_net_buf_simple_pull_u8(buf)) :
			0;
...
static void xyl_status_handle(struct bt_mesh_model *model,
			      struct bt_mesh_msg_ctx *ctx,
			      struct mesh_net_buf_simple *buf)
{
				  ...
				  struct mesh_net_buf_simple *buf)
{
	...
	status.lightness =mesh_net_buf_simple_pull_le16(buf);
	status.xy.x =mesh_net_buf_simple_pull_le16(buf);
	status.xy.y =mesh_net_buf_simple_pull_le16(buf);
				...
				struct mesh_net_buf_simple *buf)
{
	...
	status.status_code =mesh_net_buf_simple_pull_u8(buf);
	status.range.min.x =mesh_net_buf_simple_pull_le16(buf);
	status.range.max.x =mesh_net_buf_simple_pull_le16(buf);
	status.range.min.y =mesh_net_buf_simple_pull_le16(buf);
	status.range.max.y =mesh_net_buf_simple_pull_le16(buf);
...
static int bt_mesh_light_xyl_cli_init(struct bt_mesh_model *model)
{
	...
	cli->model = model;
	mesh_net_buf_simple_init_with_data(&cli->pub_msg, cli->buf,
				      sizeof(cli->buf));
...
// 	mesh_net_buf_simple_reset(cli->pub.msg);
			  ...
			  struct bt_mesh_light_xyl_status *rsp)
{
	...
	mesh_net_buf_simple_add_le16(&msg, set->params.lightness);
	mesh_net_buf_simple_add_le16(&msg, set->params.xy.x);
	mesh_net_buf_simple_add_le16(&msg, set->params.xy.y);
	mesh_net_buf_simple_add_u8(&msg, cli->tid++);
	if (set->transition) {
				  ...
				  struct bt_mesh_light_xyl *rsp)
{
	...
	mesh_net_buf_simple_add_le16(&msg, set->lightness);
	mesh_net_buf_simple_add_le16(&msg, set->xy.x);
	mesh_net_buf_simple_add_le16(&msg, set->xy.y);
				...
				struct bt_mesh_light_xyl_range_status *rsp)
{
	...
	mesh_net_buf_simple_add_le16(&msg, set->min.x);
	mesh_net_buf_simple_add_le16(&msg, set->max.x);
	mesh_net_buf_simple_add_le16(&msg, set->min.y);
	mesh_net_buf_simple_add_le16(&msg, set->max.y);
...
#endif /* CONFIG_BT_MESH_LIGHT_XYL_CLI */