...
extern void set_my_addr(u8_t index, u8_t *addr, u8_t type);
...
void send_my_proxy_config(u8_t opcode, u8_t *data)
{
    ...
    struct net_buf_simple *buf = NET_BUF_SIMPLE(29);
    struct bt_mesh_msg_ctx cur_ctx;
    ...
    M_DEBUG("%u bytes: %s", buf->len, bt_hex(buf->data, buf->len));

    err = bt_mesh_net_encode(&tx, buf, true);
    if (err) {
        M_DEBUG("Encoding Proxy cfg message failed (err %d)", err);
        return;
    }

    err = proxy_prov_segment_and_send(server->conn, BT_MESH_PROXY_CONFIG, buf);
    if (err) {
        M_DEBUG("Failed to send proxy cfg message (err %d)", err);
    }
...
void send_white_list(u8_t index)
{
    ...
    switch(index){
        case 1:
            ...
            send_my_proxy_config(CFG_FILTER_ADD, &src);
            break;

        case 2:
            send_my_proxy_config(CFG_FILTER_REMOVE, &src);
            break;
    }
    M_DEBUG("%d", index);
}
