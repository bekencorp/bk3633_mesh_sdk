...
int bt_hci_cmd_send_sync(u16_t opcode, struct net_buf *buf, struct net_buf **rsp)
{
    ...
    if (!buf) {
        ...
        if (!buf) {
            err = -ENOBUFS;
            BT_ERR("Buf create error (err %d)", err);
            goto exit;
        }
    }
    ...
    BT_DBG("opcode 0x%04x status 0x%02x\r\n", opcode, cmd(buf)->status);

    if (cmd(buf)->status) {
        BT_ERR("%s receive status error (err %d)", __func__, cmd(buf)->status);
        err = -EIO;
        net_buf_unref(buf);
    } else {
        err = 0;
...
static void read_bdaddr_complete(struct net_buf *buf)
{
    ...
    BT_DBG("status %u, %s", rp->status, bt_addr_le_str(&rp->bdaddr));
...
#else
        memcpy(bt_dev.id_addr.a.val, g_mac, 6);
        bt_dev.id_addr.type = BT_ADDR_LE_PUBLIC;
        goto set_addr;
#endif
    }
...
static int hci_init(void)
{
    ...
    err = common_init();
    if (err) {
        BT_DBG("%s, common_init err %d\n", __func__, err);
        return err;
    }

    err = le_init();
    if (err) {
        BT_DBG("%s, le_init err %d\n", __func__, err);
        return err;
    }

    err = set_event_mask();
    if (err) {
        BT_DBG("%s, set_event_mask err %d\n", __func__, err);
        return err;
    }
        ...
        const uint8_t *sd_data, size_t sd_len)
{
    ...
    if (err) {
        BT_ERR("%s, %d, set adv data fail %d \r\n\r\n", __func__, __LINE__, err);
        return err;
    }
    ...
    if (sd_len || (param->options & BT_LE_ADV_OPT_CONNECTABLE)) {
        ...
        if (err) {
            BT_ERR("%s, %d, set scan rsp data fail %d \r\n\r\n", __func__, __LINE__, err);
            return err;
        }
    }
    ...
    if (param->options & BT_LE_ADV_OPT_CONNECTABLE) {
        if (IS_ENABLED(CONFIG_BT_PRIVACY)) {
            err = le_set_private_addr();
            if (err) {
                BT_ERR("%s, %d, set private addr fail %d \r\n\r\n", __func__, __LINE__, err);
                return err;
            }
