...
struct bt_mesh_model_pub g_time_srv_pub = {
    .msg = NET_BUF_SIMPLE(2 + 20),
    .update = NULL,
};

struct bt_mesh_model_pub g_scene_srv_pub = {
    .msg = NET_BUF_SIMPLE(2 + 20),
    .update = NULL,
};

struct bt_mesh_model_pub g_scheduler_pub = {
    .msg = NET_BUF_SIMPLE(2 + 20),
    .update = NULL,
};
    ...
    for (i = 0; i < srv->state->scene_count; i++) {
        ...
        if (scene->scene_number != INVALID_SCENE_NUMBER) {
            ...
            if ((publish == false && total_len > MIN(BT_MESH_TX_SDU_MAX, BLE_MESH_SERVER_RSP_MAX_LEN)) ||
                ...
                BT_WARN("Too large scene register status. publish %d, total_len %d, msg->size %d\n",
                        publish, total_len, msg->size + BLE_MESH_SERVER_TRANS_MIC_SIZE);
                break;
            }
                         ...
                         struct net_buf_simple *buf)
{
    ...
    case BLE_MESH_MODEL_OP_SCENE_STORE_UNACK: {
        ...
        if (i == srv->state->scene_count) {
            BT_DBG("No matching Scene Number 0x%04x found, i %d, scene_count %d", scene_number, i, srv->state->scene_count);
            for (i = 0; i < srv->state->scene_count; i++) {
                scene = &srv->state->scenes[i];
                if (scene->scene_number == INVALID_SCENE_NUMBER) {
    ...
    case BLE_MESH_MODEL_OP_SCENE_DELETE_UNACK: {
        ...
        if (srv->state->target_scene == scene_number &&
            ...
            scene_model = bt_mesh_model_find(model->elem, BT_MESH_MODEL_ID_SCENE_SRV);
            if (scene_model == NULL) {
                              ...
                              struct net_buf_simple *buf)
{
    ...
    BT_ERR("year %d, month %d, day %d\n", year, month, day);
    BT_ERR("hour %d, minute %d, second %d\n", hour, minute, second);
    BT_ERR("day_of_week %d, action %d, trans_time %d, scene_number %d\n",
         day_of_week, action, trans_time, scene_number);
...
int time_scene_server_init(struct bt_mesh_model *model)
{
    if (model->user_data == NULL) {
    ...
    case BT_MESH_MODEL_ID_TIME_SETUP_SRV: {
        ...
        srv->model = model;
        BT_ERR("BT_MESH_MODEL_ID_TIME_SETUP_SRV, model id 0x%x, pub %p\n",
            model->id, model->pub);
        break;
    }
    ...
    case BT_MESH_MODEL_ID_SCENE_SETUP_SRV: {
        ...
        if (check_scene_server_init(srv->state)) {
         ...
         BT_ERR("BT_MESH_MODEL_ID_SCENE_SETUP_SRV, model id 0x%x, pub %p\n",
            model->id, model->pub);
        srv->model = model;
        break;
    }
...
int time_srv_init(struct bt_mesh_model *model, bool primary)
{
    ...
    BT_ERR("++++ %s, model %p, primary %d ++++\n", __func__, model, primary);
    /**
...
int scene_setup_srv_init(struct bt_mesh_model *model, bool primary)
{
    ...
    if (model->pub == NULL) {
        BT_ERR("scene_setup Server has no publication support");
        return -EINVAL;
    }
    return time_scene_server_init(model);
}
