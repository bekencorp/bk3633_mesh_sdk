...
static u8_t _gen_delta_analyze(struct bt_mesh_model *model, u16_t src_addr, struct net_buf_simple *buf)
{
    ...
    if(mesh_check_tid(src_addr, tid) == MESH_TID_REPEAT) {
        ...
        elem->state.trans_level = elem->state.gen_level[T_CUR];
        target = elem->state.gen_level[T_CUR] + delta;
    }
...
#endif
    if(delta_move > 0) {
        elem->state.gen_level[T_TAR] = (s16_t)0x7FFF;
    } else {
...
const struct bt_mesh_model_op g_gen_level_op[GEN_LV_OPC_NUM] = {
    { BT_MESH_MODEL_OP_GEN_LEVEL_GET,       0, _gen_level_get },
    { BT_MESH_MODEL_OP_GEN_LEVEL_SET,       3, _gen_level_set },
    { BT_MESH_MODEL_OP_GEN_LEVEL_SET_UNACK, 3, _gen_level_set_unack },
    { BT_MESH_MODEL_OP_GEN_DELTA_SET,       5, _gen_delta_set },
    { BT_MESH_MODEL_OP_GEN_DELTA_SET_UNACK, 5, _gen_delta_set_unack },
    { BT_MESH_MODEL_OP_GEN_MOVE_SET,        3, _gen_move_set },
    { BT_MESH_MODEL_OP_GEN_MOVE_SET_UNACK,  3, _gen_move_set_unack },
...
void bound_level_by_onoff(S_ELEM_STATE *elem)
{
    ...
    {
        elem->state.gen_level[T_CUR] = elem->powerup.last_level;
    }
}
