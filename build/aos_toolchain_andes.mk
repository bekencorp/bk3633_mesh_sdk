ANDES_GNU_ARCH_LIST := n10  \
                       n8   \
		       andes

ifneq ($(filter $(HOST_ARCH), $(ANDES_GNU_ARCH_LIST)),)

TOOLCHAIN_PATH    ?=
ifneq ($(filter $(HOST_MCU_FAMILY), ab161x), )
TOOLCHAIN_PREFIX := nds32le-elf-
TOOLCHAIN_DEFAULT_FOLDER := nds32le-elf-mculib-v3m
#TOOLCHAIN_PREFIX := arm-none-eabi-
#TOOLCHAIN_DEFAULT_FOLDER := gcc-arm-none-eabi
else

TOOLCHAIN_PREFIX  := nds32le-elf-
TOOLCHAIN_DEFAULT_FOLDER := gcc-andes
endif
...
COMPILER_SPECIFIC_OPTIMIZED_CFLAGS    := -Og
...
COMPILER_SPECIFIC_DEBUG_ASFLAGS    := -DDEBUG=1 -ggdb $(COMPILER_SPECIFIC_UNOPTIMIZED_CFLAGS)
...
COMPILER_SPECIFIC_RELEASE_LOG_ASFLAGS  := -ggdb $(COMPILER_SPECIFIC_OPTIMIZED_CFLAGS)
...
COMPILER_SPECIFIC_RELEASE_ASFLAGS  := -ggdb $(COMPILER_SPECIFIC_OPTIMIZED_CFLAGS)
...
ifeq ($(HOST_ARCH),n10)
CPU_CFLAGS   := -mcmodel=large -mno-ifc -mno-ex9
CPU_CXXFLAGS := $(CPU_CFLAGS)
CPU_ASMFLAGS := $(CPU_CFLAGS)
CPU_LDFLAGS  := $(CPU_CFLAGS)
endif
...
endif #ifneq ($(filter $(HOST_ARCH), n10),)
