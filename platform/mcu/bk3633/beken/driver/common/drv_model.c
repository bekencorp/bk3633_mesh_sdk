...
static uint8_t drv_info_flag = 0;

void drv_info_enable(uint8_t info_en)
{
	drv_info_flag = info_en;
}
...
DD_HANDLE ddev_open(char *dev_name, UINT32 *status, UINT32 op_flag)
{
    ...
    if((dev_name[0]!='u')&&(dev_name[1]!='a')&&(dev_name[2]!='r')&&drv_info_flag)
        os_printf("ddev_open--%s", dev_name);
...
UINT32 ddev_read(DD_HANDLE handle, char *user_buf, UINT32 count, UINT32 op_flag)
{
    ...
    if(drv_info_flag)	os_printf("ddev_read--0x%x, %d\r\n", handle, count);
...
UINT32 ddev_write(DD_HANDLE handle, char *user_buf, UINT32 count, UINT32 op_flag)
{
    ...
    if(drv_info_flag)	os_printf("ddev_write--0x%x, %d\r\n", handle, count);
...
UINT32 sddev_control(char *dev_name, UINT32 cmd, VOID *param)
{
	...
	if((dev_name[0]!='i')&&(dev_name[1]!='c')&&(dev_name[2]!='u')&&drv_info_flag)
        os_printf("sddev_control--%s\r\n", dev_name);
