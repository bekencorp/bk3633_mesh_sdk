...
enum adv_dup_filt_info
{
    ...
    LLM_ADV_DUP_FILT_CHAIN_1M_LSB      = 1,
    LLM_ADV_DUP_FILT_CHAIN_1M_MASK     = (0x3 << 1),

    /// Advertising report chain status from coded scan
    LLM_ADV_DUP_FILT_CHAIN_CODED_LSB   = 3,
    LLM_ADV_DUP_FILT_CHAIN_CODED_MASK  = (0x3 << 3),
...
struct llm_per_adv_info
{
     ...
     *    - ICL Master stream linked to periodic advertising
     */
    uint8_t                         ass_act_id;
};
...
struct llm_env_tag
{
    ...
    /// Indicate Host requires accurate clock
    bool host_clk_acc;
};
