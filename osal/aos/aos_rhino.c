...
int aos_task_new_static(aos_task_t *task, const char *name, void (*fn)(void *),
                     void *arg, cpu_stack_t *stack_buf, int stack_size, int prio)
{
    int ret;

    ktask_t *task_obj = NULL;
    ...
    task->hdl = task_obj;
    ret = (int) krhino_task_create((ktask_t *)task->hdl, name, arg,
                            prio, 0, stack_buf, stack_size/sizeof(cpu_stack_t), fn, 1);
